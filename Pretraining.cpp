...
float stringToFloat(const string& str) {
    ...
    // 整数部分
    for (int i = startIndex; i < (decimalPointIndex == string::npos ? str.length() : decimalPointIndex); i++) {
        result = result * 10 + (str[i] - '0');
    }

    // 小数部分
    if (decimalPointIndex != string::npos) {
        float decimalFactor = 0.1; // 用于小数部分的权重
        for (int i = decimalPointIndex + 1; i < str.length(); i++) {
            result += (str[i] - '0') * decimalFactor;
            decimalFactor /= 10; // 逐步减小权重
        }
    }
